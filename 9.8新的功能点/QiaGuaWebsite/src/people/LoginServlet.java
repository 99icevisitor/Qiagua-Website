...
public class LoginServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameter("userName")!=null && request.getParameter("userPwd")!=null){
			/*初步生成用户信息
			 * 需要验证用户名是否和登录表里的用户名相同，如果相同，则不能完成初步注册
			 * 注：仅完成‘初步注册’的用户会在两天之内被系统注销！！
			 * try:  Integer userID = createUser(userName,userPwd,0);
			 *       √在LOGIN表生成一条记录：USER_ID,USER_NAME,PWD,LAST_TIME,STATE="初步注册，未绑定邮箱和手机号"
			 *       ×此时用户还不在USER表中，如果注册未完成，则用户可登录，但是不在有效用户中
		...
		if(request.getParameter("userMail")!=null){
			/*进一步完善用户信息
			 * Integer ID = request.getAttribute("ID");
			 * String mail = request.getParameter("userMail");
			 * String tell = request.getParameter("userTell");
			 * try:  addUserInfo(ID,mail,tell);
			 *       √在USER表中生成一条记录：USER_ID,USER_NAME,PWD,MAIL,MOBILE;
			 *       √更新LOGIN表中LAST_TIME、STATE="已经绑定邮箱和手机号,未实名认证"
			 * */
